...
const Setup: React.FC<SetupProps> = ({ API_URL }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_URL}/initialize-setup`, {
                method: 'POST',
            ...
            <div className="form-container" style={{maxWidth: '600px'}}>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        <label>Datenbank Host/IP</label>
                        <input name="dbHost" value={formData.dbHost} onChange={handleChange} required />
                    </div>
                    <div className="form-group">
                        <label>Datenbank Benutzer</label>
                        <input name="dbUser" value={formData.dbUser} onChange={handleChange} required />
                    </div>
                    <div className="form-group">
                        <label>Datenbank Passwort</label>
                        <input type="password" name="dbPassword" value={formData.dbPassword} onChange={handleChange} />
                    </div>
                    <div className="form-group">
                        <label>Datenbank Name</label>
                        <input name="dbName" value={formData.dbName} onChange={handleChange} required />
                    </div>
                    ...
                    <button type="submit" className="primary-button" style={{width: '100%', marginTop: '1rem'}}>Setup abschließen</button>
                </form>
            </div>
        </div>
    );
